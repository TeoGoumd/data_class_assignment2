...
nyt_salary <- MediaBucks %>%
  group_by(COMPANY == "NYT") %>%
...
MediaBucks %>%
  select(Gender, Salary, Race, TITLE, COMPANY) %>%
  filter(COMPANY == "NYT") %>%
  group_by(TITLE) %>%
  arrange(desc(Salary))
...
```{r}
my_choice <- filter(MediaSalary, COMPANY %in% c("CNN", "Politico", "VoxMedia"))
...
result <- MediaBucks %>%
filter(Salary == 230504 & TITLE == "reporters")
...
editors <- MediaBucks %>%
  ...
  median_salary <- median(editors$Salary, na.rm = TRUE)
...
selected_positions <- MediaBucks %>%
  filter(grepl("data", TITLE, ignore.case = TRUE)) %>%
  select(Salary, YEARS_EXPERIENCE)
